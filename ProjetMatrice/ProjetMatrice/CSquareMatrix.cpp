	...
	for(unsigned int uiRow = 0; uiRow < oSMTXmatrixParam.MTXgetHeight(); uiRow++)
		for(unsigned int uiColumn = 0; uiColumn < oSMTXmatrixParam.MTXgetWidth(); uiColumn++)
		{
			if(uiRow == uiRowParam || uiColumn == uiColumnParam) //On ne considère pas la ligne (resp. colonne) s'il s'agit de la ligne (resp. colonne) que l'on veut exclure
				continue;
			ptValues[uiPosition / uiHeight][uiPosition % uiHeight] = oSMTXmatrixParam.MTXgetValue(uiRow, uiColumn); //Recopie dans la matrice en cours (avec uiPosition) les valeurs de celle en paramètre (avec uiRow et uiColumn)
			uiPosition++;
		}
}
...
CSquareMatrix<T>::CSquareMatrix(unsigned int uiSize, char * pcMethod) : CMatrix<T>(uiSize, uiSize)
{
	if(STRCMPI(pcMethod, "eye") == 0 || STRCMPI(pcMethod, "eyes") == 0)
		for(unsigned int uiRow = 0; uiRow < uiSize; uiRow++)
			MTXsetValue(uiRow, uiRow, 1); //Affecte 1 à toutes les cases de la diagonale
}
...
{
	if(uiHeight == 1)
		return ptValues[0][0]; //S'il s'agit d'une matrice 1;1 on renvoie son unique valeur
	else if(uiHeight == 2)
		return ptValues[0][0] * ptValues[1][1] - ptValues[1][0] * ptValues[0][1]; //S'il s'agit d'une matrice 2;2 on calcul et renvoie son déterminant
	else
	{
...
CSquareMatrix<T>& CSquareMatrix<T>::SMTXpow(unsigned int power)
{
	...
	return (SMTXpow(power - 1) *= (*this)); //Multiplication de la matrice actuelle par la matrice à la puisance power-1 par appel récursif de la méthode
}
...
{
	CSquareMatrix<T> * poSMTXtrans = new CSquareMatrix(uiWidth); //Créé une nouvelle matrice
	for(unsigned int uiRow = 0; uiRow < uiHeight; uiRow++)
		for(unsigned int uiColumn = 0; uiColumn < uiWidth; uiColumn++)
			poSMTXtrans->MTXsetValue(uiColumn, uiRow, MTXgetValue(uiRow, uiColumn)); //Recopie dans la nouvelle matrice les valeurs de l'originale en inversant lignes et colonnes
...
CSquareMatrix<T>& CSquareMatrix<T>::operator= (CSquareMatrix<T> const& oSMTXmatrixParam)
{
	...
	for (unsigned int uiRow = 0; uiRow < uiHeight; uiRow++)
		for (unsigned int uiColumn = 0; uiColumn < uiWidth; uiColumn++)
			ptValues[uiRow][uiColumn] = oSMTXmatrixParam.MTXgetValue(uiRow, uiColumn); //Recopie dans chaque case de la nouvelle matrice la valeur de la case correspondante de la matrice passée en paramètre
...
CSquareMatrix<T>& CSquareMatrix<T>::operator*= (CSquareMatrix<T> const& oSMTXmatrixParam)
{
	...
	if (uiHeight != oSMTXmatrix.MTXgetHeight() || uiWidth != oSMTXmatrix.MTXgetWidth())
		throw CException(INCOMPATIBLE_MATRIX_EXCEPTION, (char *) "The two matrix don't have the same size");
...
CSquareMatrix<T>& CSquareMatrix<T>::operator*= (double dCoefficient)
{
	for(unsigned int uiRow = 0; uiRow < uiHeight; uiRow++)
		for(unsigned int uiColumn = 0; uiColumn < uiWidth; uiColumn++)
			MTXsetValue(uiRow, uiColumn, ptValues[uiRow][uiColumn] * dCoefficient); //Affecte à chaque case sa valeur multipliée par le coefficient passé en paramètre
	return *this;
}
